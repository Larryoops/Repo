...
public class PedometerService extends Service {
	...
	public void SensorEvent() {
		...
		sensorEventlistener = new SensorEventListener() {
			...
			public void onSensorChanged(SensorEvent event) {
				...
				/**/
				// accelerometerValues = new double[4];
				// orientationValues = new double[4];
				...
				case Sensor.TYPE_ACCELEROMETER:
					...
					//System.out.println("x : " + accelerometerValues[0]);
					//System.out.println("y : " + accelerometerValues[1]);
					//System.out.println("z : " + accelerometerValues[2]);
					...
					if (primitiveAccelerometerList.size() > 300) {
						stepNumAndstrideLength = DataHandling
								.getStepNumsAndTotalLength(primitiveAccelerometerList);
						...
						offlineList.addAll(primitiveAccelerometerList.subList(
								35, 300));
						for (int i = 0; i < 265; i++) {
				...
				case Sensor.TYPE_ORIENTATION:
					...
					roll = orientationValues[0];
					curTime = System.currentTimeMillis();

					//System.out.println("水平方向角 : " + orientationValues[0]);
					// System.out.println("上下俯仰角 : "+orientationValues[1]);
					// System.out.println("左右翻轉角 : "+orientationValues[2]);
		...
		sm.registerListener(sensorEventlistener, accelerometer,
				SensorManager.SENSOR_DELAY_FASTEST);
		sm.registerListener(sensorEventlistener, orientation,
				SensorManager.SENSOR_DELAY_FASTEST);
	}
	...
	public double[] getStepAndStrideAndOrientation()
	{
		return new double[]{stepNum,getTotalLength() / getStepNum() * 2,roll};
	}
	...
	public double getAccuracyStrideLength() {
		return getAccuracyStepNumAnsTotalLength()[1]
				/ getAccuracyStepNumAnsTotalLength()[0] * 2;
	}

	public double getAccuracyVelocity() {
		return getAccuracyStepNumAnsTotalLength()[1] / (curTime - firstTime)
				* 1000 * 3600 / 1000;
	}

	public List<double[]> getVelocity1() throws MathException, IOException {
		return DataHandling.CalculateVelocityAndDisplacement(offlineList,
				accelerometerList, orientationList);
	}

	public List<double[]> getRotatedAccelerometer() throws MathException {
		return DataHandling.getRotatedAccelerometer(accelerometerList,
				orientationList);
	}

	public List<Double> getLocalMeanAccelerometer() {
		return DataHandling.getLocalMeanAcceleration(DataHandling
				.getMagnitudeOfAcceleration(offlineList));
	}
